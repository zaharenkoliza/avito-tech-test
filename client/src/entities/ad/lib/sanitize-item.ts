...
const categorySanitizers: Record<Category, (params: ParamsValue | undefined) => ParamsValue> = {
	auto: (params) => ({
		brand: pickString(params, 'brand'),
		model: pickString(params, 'model'),
		yearOfManufacture: pickNumber(params, 'yearOfManufacture'),
		transmission: pickEnum(params, 'transmission', ['automatic', 'manual'] as const),
...
export const sanitizeItemForCategory = (item: ItemWithRevision): ItemWithRevision =>
	({
		...item,
