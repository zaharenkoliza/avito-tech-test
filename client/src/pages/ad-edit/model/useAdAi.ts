...
export const useAdAi = ({ adId }: UseAdAiParams) => {
	...
	const [descriptionRequestError, setDescriptionRequestError] =
		useState<string>();
	...
	const aiControllersRef = useRef<
		Partial<Record<AIRequestKind, AbortController>>
	>({});
	...
	useEffect(() => {
		const chatHistoryByAd = storage.get<Record<number, ChatMessage[]>>(
			CHAT_KEY,
			{},
		);
	...
	const finishAIRequest = useCallback(
		(kind: AIRequestKind, controller: AbortController) => {
	...
	const suggestPrice = useCallback(
		async (item: ItemWithRevision) => {
			...
			try {
				const response = await aiService.suggestPrice(
					{ item },
					controller.signal,
				);
