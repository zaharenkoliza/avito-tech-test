import { Box, Group, Paper, Select, TextInput, UnstyledButton, useMantineColorScheme } from '@mantine/core'
		...
		>
			<Group gap={12} wrap="nowrap" align="center">
				<TextInput
					...
					radius={8}
					rightSection={<IconSearch size={16} stroke={1.7} color={isDark ? '#9ea7b6' : '#666'} />}
					styles={{
						input: {
							height: 36,
				...
				>
					...
					>
						<IconLayoutGrid size={16} color={layout === 'grid' ? '#2f8cff' : isDark ? '#c4cad4' : '#2f2f2f'} />
					</UnstyledButton>
					...
					>
						<IconList size={16} color={layout === 'list' ? '#2f8cff' : isDark ? '#c4cad4' : '#2f2f2f'} />
					</UnstyledButton>
				</Box>
				...
				<Select
					...
					styles={{
						input: {
							height: 36,
							backgroundColor: isDark ? 'var(--mantine-color-dark-6)' : '#ffffff',
							borderColor: isDark ? 'rgba(255, 255, 255, 0.08)' : '#efebe7',
							fontSize: 14,
						},
					}}
